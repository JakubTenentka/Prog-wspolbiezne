            ...
            Task.Run(async () =>
            {
                ...
                while (Animating && round == roundCounter)
                {
                    lock (ball)
                    {
                        ...
                        else if (ball.X + ball.Radius > dataAPI.Backgroundwidth)
                        {
                            ball.X = dataAPI.Backgroundwidth - ball.Radius;
                        }
                        if (ball.Y - ball.Radius < 0)
                        {
                            ball.Y = ball.Radius;
                        }
                        else if (ball.Y + ball.Radius > dataAPI.Backgroundheight)
                        {
                            ball.Y = dataAPI.Backgroundheight - ball.Radius;
                        }
                    }
